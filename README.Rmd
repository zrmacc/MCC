...
* `@CIs` containing confidence intervals for the difference and ratio of AUCs.
...
The influence function contributions are useful for estimating the variance of $\hat{\theta}(\tau)$:

$$
\hat{\mathbb{V}}(\hat{\theta}) = \frac{1}{n^2}\sum_{i=1}^{n}\psi_{i}^{2}(\tau).
$$

`InfluenceFunction` can be used to calculate the influence contributions of each subject to the MCF (`type = "MCF"`) or AUMCF (`type = "AUC"`) at a given time $\tau$. 
